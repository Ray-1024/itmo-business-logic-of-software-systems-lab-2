    ...
    public SecurityFilterChain filterChain(HttpSecurity http, UserDetailsService userDetailsService, JwtJAASFilter jwtJAASFilter) throws Exception {
        return http
                ...
                .authorizeHttpRequests(c -> c
                        ...
                        .requestMatchers(POST, "/api/client/order").hasRole(Role.CLIENT.name())
                        .requestMatchers(GET, "/api/packer/order").hasRole(Role.PACKER.name())
                        .requestMatchers(PUT, "/api/packer/order").hasRole(Role.PACKER.name())
                        .requestMatchers(GET, "/api/courier/order").hasRole(Role.COURIER.name())
                        .requestMatchers(PUT, "/api/courier/order").hasRole(Role.COURIER.name())
